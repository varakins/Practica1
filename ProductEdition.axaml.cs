...
public partial class ProductEdition : Window
{
    ...
    // Метод для загрузки данных товара для редактирования
    public void LoadProductData(int productId)
    {
        ...
        using (var conn = new NpgsqlConnection(connectionString))
        {
            ...
            string query = @"
                SELECT id, articl, title_id, unit_of_measurement, price, 
                       suplitiers_id, manufactures_id, categories_products, 
            ...
            using (var cmd = new NpgsqlCommand(query, conn))
            {
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        _articlTextBox.Text = reader.GetString(1);
                        
                        // Выбираем соответствующие элементы в выпадающих списках
                        SelectComboBoxItemById(_titleComboBox, reader.GetInt32(2));
                        SelectComboBoxItemById(_categoryComboBox, reader.GetInt32(7));
                        SelectComboBoxItemById(_manufacturerComboBox, reader.GetInt32(6));
                        SelectComboBoxItemById(_supplierComboBox, reader.GetInt32(5));
                        
                        _unitTextBox.Text = reader.GetString(3);
                        _priceTextBox.Text = reader.GetDecimal(4).ToString("F2");
                        _quantityTextBox.Text = reader.GetInt32(9).ToString();
                        _discountTextBox.Text = reader.GetInt32(8).ToString();
                        _descriptionTextBox.Text = reader.IsDBNull(10) ? "" : reader.GetString(10);
                        
                        // Загружаем изображение
                        _originalImageFileName = reader.IsDBNull(11) ? null : reader.GetString(11);
                        if (!string.IsNullOrEmpty(_originalImageFileName))
                        {
                            try
                            {
                                string imagePath = System.IO.Path.Combine(
                                    AppDomain.CurrentDomain.BaseDirectory, 
                                    "..", "..", "..", "Assets", _originalImageFileName);
                                
                                if (File.Exists(imagePath))
                                {
                                    var bitmap = new Bitmap(imagePath);
                                    _productImage.Source = bitmap;
                                    _imageFileNameText.Text = _originalImageFileName;
    ...
    private async void OnSaveButtonClick(object sender, RoutedEventArgs e)
    {
        ...
        string imageFileName = _originalImageFileName;
        ...
        try
        {
            ...
            await ShowMessage("Успех", "Товар успешно обновлен");
        ...
        {
            await ShowMessage("Ошибка", $"Не удалось обновить товар: {ex.Message}");
        }
    }
